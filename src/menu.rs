...
impl<'a, T> Menu<'a, T> {
    pub fn new(heading: &'static str) -> Self {
        Self {
            heading: Text::default().text(heading),
            list: vec![],
            cursor: 0,
            id: 0,
            routine_fn: None,
    ...
    pub fn action(
        ...
        self.list
            .push(MenuItem::<'a>::Action(Action { label, action_fn }));
        self
    }

    pub fn sub_menu(mut self, label: &'a str, mut sub_menu: Menu<'a, T>) -> Self {
        sub_menu.id = self.id + 1;
        self.list.push(MenuItem::<'a>::SubMenu(SubMenu {
            menu: sub_menu,
            label,
        }));
        self
    ...
    pub fn back(mut self, label: &'a str) -> Self {
        self.list.push(MenuItem::<'a>::Action(Action {
            label,
            action_fn: &|_, _| -> Return { Return::Back },
        }));
        self
    }
    ...
    pub fn run(&mut self, terminal: &mut TerminalHandler, ctx: &mut T) -> Return {
        let mut menu_win = terminal.root.new_child(
            Window::default().size(terminal.root.width(), (self.list.len() * 2 + 4) as u32),
        );
        let mut options_win =
            menu_win.new_child(Window::default().size(40, (self.list.len() * 2 - 1) as u32));

        let mut opt = Text::default().start_text((1, 0));
        let run_routine = |s: &mut Self, ctx: &mut T| match s.routine_fn {
            Some(r) => r(s, ctx),
            None => (),
        };
        ...
        loop {
            ...
            for (idx, o) in self.list.iter().enumerate() {
                ...
                opt.set_size(label.len() as u32 + 2, 1);
                opt.set_text(label);
                opt.set_fg(Color::Reset);
                opt.set_xy(0, (idx * 2) as u32);
            ...
            match terminal.event() {
                Event::Key(k) => match k.code {
                    ...
                    KeyCode::Down => {
                        self.cursor += 1;
                        if self.cursor == self.list.len() as u32 {
                            self.cursor = 0;
                        }
                    }
                    KeyCode::Up => {
                        if self.cursor == 0 {
                            self.cursor = (self.list.len() - 1) as u32;
                        } else {
                            self.cursor -= 1;
                        }
                    }
                    KeyCode::Enter => {
                        ...
                        match menu_item {
                            MenuItem::Action(a) => match (a.action_fn)(terminal, ctx) {
                                Return::ToRoot => {
                                    if self.id != 0 {
                                        RETURN!(ToRoot);
                                    }
                            ...
                            MenuItem::SubMenu(sm) => match sm.menu.run(terminal, ctx) {
                                Return::ToRoot => {
                                    if self.id != 0 {
                                        RETURN!(ToRoot);
                                    }
